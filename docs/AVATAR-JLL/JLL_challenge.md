...
The following trajectories are provided as [list of joint positions](JLL_doc#241-robot-monitoring) defined in a JSON file:
...
**Output:** 1) animation files of the 4 trajectories for VEB.js or equivalent configuration of a different VR environment to play the animations.

**Hints:** the input trajectories can be elaborated using any programming language, e.g. Python, C++, Java, JavaScript, etc.

### Task T.1.3
Visually check the animation behaviour in the selected VR environment.

**Output:** 1) Video of the 4 trajectories in the selected VR environment.
...
## *Trajectory*
The trajectory for this challenge is not provided in a json file, but it can be received as separate messages via [MQTT communication](JLL_doc#25-mqtt-communication). The messages are generated by an emulator of the robot controller.
...
## *Trajectory*
The trajectory for this challenge is not provided since it must be generated. The starting position is defined by the following [list of joint positions](JLL_doc#241-robot-monitoring):
...
The [target position](#trajectories) is the same as for the other challenges.
