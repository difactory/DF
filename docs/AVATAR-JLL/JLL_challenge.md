...
The messages are generated by an emulator of the robot controller that can be launched using the web application [OntoGuiWeb](https://virtualfactory.gitbook.io/vlft/tools/ontoguiweb).  Specifically, the module [MQTT Sync](https://virtualfactory.gitbook.io/vlft/tools/ontoguiweb/modules/mqtt-sync) provides functionalities as an MQTT client to publish messages and subscribe to topics. The trajectory of this challenge can be generated by pushing the button "Start Publish" on the right of the text "Challenge#2 Sequence", thus emulating the generation of messages that would do the robot controller. It is recommended to **personalize the Topic to publish the message sequence**, since other people could use this functionality at the same time.
...
The starting position is defined by the following [list of joint positions](JLL_doc#241-robot-monitoring):
